...
<h2>These are loans that are currently pending.</h2>
<h5>&nbsp;&nbsp;&nbsp;Click the index number in the left column to view the loan, or click the copy loan link in the right column to copy the loan into a new reservation.</h5>
...
<table class="grid">
	...
	{foreach from=$reservations item=reserve key=id}
		<tr>
			<td><a class="button" href="{$PHP.BASE_URL}/history/search/id/{$id}">View Loan</a></td>
			<td>{$reserve.lname}</td>		
			<td>{$reserve.fname}</td>
			...
			<td><a class="button" href="{$PHP.BASE_URL}/history/copy/{$id}">Copy Loan</a>
		</tr>
	{/foreach}
	</tbody>
</table>
