...
{col size=10}
	...
	{box title="Assign Ticket To" class="noprint"}
		<ul>
			...
			<button type="submit" class="btn" name='toggle' value='1'>{if $call_status == 'open' || !$call_id}Update and Close {else if $call_status == 'closed'}Update and Reopen{/if} call</button>
			<button type="submit" class="btn btn-primary" name='toggle' value='0'>{if $call_id}Update{else}Submit{/if} Ticket</button>
			</li>
		</ul>
	{/box}
